        ...
        class WC_send_pdf {
             ...
             */
		 	function pdf_attachment( $attachment = NULL, $id = NULL, $order = NULL ) {
				...
				// Attach to the completed order email
			...
			Public Static function get_woocommerce_invoice( $order = NULL, $stream = NULL ) {
				...
				/**
				 * Sort the file name based on the settings
				 *
				 * Allowed variables
				 *
				 * companyname
				...
				if ( $stream && ( !isset($woocommerce_pdf_invoice_options['pdf_termsid']) || $woocommerce_pdf_invoice_options['pdf_termsid'] == 0 ) ) {
				...
				} elseif ( isset($woocommerce_pdf_invoice_options['pdf_termsid']) && $woocommerce_pdf_invoice_options['pdf_termsid'] != 0 ) {
					/**
					...
					foreach ( $order->get_items() as $item ) {
						...
						if ( $item['qty'] ) {
							...
							$line =  	'<tr>' .
										...
										'<td valign="top" width="7%" align="right">'  .  woocommerce_price( $item['line_tax'] / $item['qty'] ) . '</td>' .			
										'<td valign="top" width="10%" align="right">' .  woocommerce_price( ( $item['line_subtotal'] + $item['line_tax'] ) / $item['qty'] ) . '</td>' .
			...
			function get_woocommerce_invoice_terms( $page_id = 0 ) {
				 ...
				 */
				$plugin_version     = str_replace('/classes/','/templates/',plugin_dir_path(__FILE__) ) . 'terms-template.php';
                $theme_version_file = get_stylesheet_directory() . '/pdf_templates/' . 'terms-template.php';
				
				$termstemplate 		= file_exists($theme_version_file) ? $theme_version_file : $plugin_version;
